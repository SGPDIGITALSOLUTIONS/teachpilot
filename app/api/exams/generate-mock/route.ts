    ...
    userPrompt += `\n\nReturn questions in JSON format:
{
  "questions": [
    ...
    // Store exam in database (mock exams are stored but marked differently)
    // Use a transaction to handle potential race conditions and ensure unique version numbers
    ...
    try {
      ...
      const examResult = await client.query(
        ...
        [
          ...
          topic_id,
          finalVersionNumber,
          `Mock Exam - ${material.title} (v${finalVersionNumber})`,
        ...
        try {
          ...
          const retryExamResult = await pool.query(
            ...
            [
              ...
              topic_id,
              retryVersionNumber,
              `Mock Exam - ${material.title} (v${retryVersionNumber})`,
              JSON.stringify(validQuestions),
              validQuestions.length
            ]
          );
    ...
    const errorMessage = error.message || 'Failed to generate mock exam';
    return NextResponse.json(
      { 
        success: false, 
        message: isNetworkError
          ...
          : errorMessage.includes('API key') 
            ...
            : `Failed to generate mock exam: ${errorMessage}` 
      },
      { status: 500 }
    );
  }
